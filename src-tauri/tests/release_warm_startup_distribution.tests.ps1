param(
    ...
    [double]$ReleaseCeilingMilliseconds = 1500
)
...
Write-Output ("Warm $ShellKind startup median: {0:N1} ms; p95: {1:N1} ms; maximum: {2:N1} ms." -f $median, $p95, $maximum)
Write-Output ("Raw samples (ms): " + (($samples | ForEach-Object { $_.ToString("F1", [Globalization.CultureInfo]::InvariantCulture) }) -join ", "))
if ($maximum -gt $ReleaseCeilingMilliseconds) {
    throw "Warm $ShellKind startup maximum $maximum ms exceeded the $ReleaseCeilingMilliseconds ms release ceiling."
}
