...
`head`, finite `tail -n N`, `wc -l`, and `grep` record stream for `>` and `>>`. Integration tests cover input-before-
...
test now also exercises Unicode-path `cat | head >`, `wc -l >`, `tail -n 1 >`, and Unicode `grep -n >` submissions through this
