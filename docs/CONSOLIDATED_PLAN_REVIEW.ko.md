...
상태: 처음부터 끝까지 검토를 마쳤고 설계 방향을 조건부로 승인한다. 이 문서는
구현을 **허가하지 않는다**.
...
검토에서 계약 blocker 4개와 가치가 큰 수정 6개를 찾았고 문서 수준에서는 열 가지를
모두 닫았다. 그래도 구현을 허가한 것은 아니다. 사용자가 통합 계획을 다시 검토하고
명시적으로 승인해야 하며 경계 기술 검증은 구현의 첫 단계로 남는다.

현재 초안은 유용한 마이그레이션 기준이다. 2026-08-05 기준 TypeScript,
PowerShell, cmd, layout, Rust 기존 테스트가 모두 통과했다. 이는 현재 초안이
내부적으로 안정적이라는 증거이지 새 공통 해석기 계약의 합격 증거는 아니다.
