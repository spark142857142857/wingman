...
현재 초안에는 신뢰할 수 있는 startup/readiness marker, 전체 process tree 계산,
결정적인 PTY 부하 생성, runner timing, 자원 제한, 지속 실행 자동화가 없다. 이는
계획된 측정 요구사항이며 구현 승인 전에 제품 계측을 추가해도 된다는 뜻은 아니다.
