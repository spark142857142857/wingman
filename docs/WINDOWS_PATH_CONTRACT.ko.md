...
Production sidecar는 이제 이 primitive를 검증된 `cat`·`head`·유한 `tail -n N`·`wc -l`·`grep` record stream의 `>`·`>>`에
...
`cat | head >`·`wc -l >`·`tail -n 1 >`·Unicode `grep -n >` 제출을 이 primitive까지 연결한다. Familiar OFF, Uncertain, 명시적 native,
