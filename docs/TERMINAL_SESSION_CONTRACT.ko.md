...
상태: 현재 P0 터미널·세션 계약이며 릴리스 후보에 구현되어 있다.
...
**현재 구현 상태(2026-08-20):** OOB 채널은 production PowerShell session에
연결됐고, 반복 ConPTY PowerShell → readiness → Rust 판정 → request broker →
실제 sidecar → 다음 readiness 수직 테스트를 통과했다. Production에는 PTY
readiness parser가 없다. Familiar 기본값은 `PAUSED`로 유지하지만 명시적
`familiar on`은 typed Rust runner를 통해 pipeline과 최종 output redirection을 포함한
계약된 P0 catalog를 활성화한다. Familiar OFF, Uncertain editing, `cmd`는 native를 유지한다.
앞선 경계 테스트에서
